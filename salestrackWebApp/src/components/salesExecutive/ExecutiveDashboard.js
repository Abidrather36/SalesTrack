...
export default function ExecutiveDashboard() {
  ...
  const manageLead = (lead) => {
    console.log("manageLead called with lead:", lead);
    setLeadData(lead);
    setFollowUpdatePopup(true);
  };

  const followUpHistoryLead = (lead) => {
    console.log("followUpHistoryLead called with:", lead);
    if (!user || !lead?.leadId) {
      myToaster.showErrorToast("Unable to navigate to follow-up history.");
      return;
    }
    const basePath = user.userRole === 3 ? "/salesExecutive" : "/salesManager";
    navigate(`${basePath}/followUphistory/${lead.leadId}`);
  };
        ...
        if (result.length === 0) {
          if (isCustomSearch) {
            myToaster.showErrorToast(
              "No follow-up history found for the selected date."
            );
          }
        ...
        <div style={{ marginRight: "400px" }} className="col-lg-6 ml-3">
          <form
            ...
            onSubmit={handleSubmit((data) =>
              onfetchFollowUpHistory(data, true)
            )}
            autoComplete="off"
          >
            <div
      ...
      <div>
        {!loading && hasFollowUpHistory ? (
          <Grid
            buttons={[
              ...
              },
              {
                ...
                onAddFollowUpHistory: (lead) => followUpHistoryLead(lead), // This is correct
                icon: <FaHistory />,
              },
            ]}
            headers={headers}
          ...
          <p style={{ marginLeft: "30px" }}>
            No follow-up history found Today.
          </p>
        ) : null}
      </div>
