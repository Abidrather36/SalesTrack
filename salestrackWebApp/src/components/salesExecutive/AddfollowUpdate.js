...
//         // Format the follow-up dates in the history as 'dd-mm-yyyy' using split('T')
...
//             // Reformat the date to 'dd-mm-yyyy' if date exists
...
//             };
//           })
//           // Sort the history by followUpDate in descending order (most recent first)
...
//     setOpen(false);
//     onClose();
//     setShowList(true); // Ensure we are showing the follow-up list
//   };

//   const handleManageLeadClick = () => {
//     setOpen(true); // Directly open the modal
//     setShowList(false); // Hide the follow-up history when showing the modal
//   };
...
export default function BasicModal({ leadData, onClose,showFallowup,showFoloowUpHistory }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      myToaster.showErrorToast("Something went wrong,try again:");
